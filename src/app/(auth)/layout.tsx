...
function AuthLayoutContent({
  ...
  const isSignIn = pathname.includes("/sign-in") || pathname.includes("/(auth)/sign-in") || pathname.includes("/module/auth/sign-in");
  title = title ?? (isSignIn ? "Welcome back!" : "Create your account!");
  const auth_background = 'https://res.cloudinary.com/dmdzyoslx/image/upload/v1763100684/avatars/73879289-c33d-4d86-890a-60f0af7fd9e1.jpg';
  ...
  const header = (
    <div className="flex items-center gap-6 text-sm text-zinc-500">
      <button
        className={`pb-2 ${tab === "username" ? "border-b-2 border-zinc-900 dark:border-zinc-100 text-zinc-900 dark:text-zinc-100" : ""
          }`}
      ...
      <button
        className={`pb-2 ${tab === "mobile" ? "border-b-2 border-zinc-900 dark:border-zinc-100 text-zinc-900 dark:text-zinc-100" : ""
          }`}
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-black p-8">
      <div className="relative flex w-full max-w-5xl rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div
          ...
          style={{
            background:
              `linear-gradient(180deg, rgba(255,255,255,0.12), rgba(255,255,255,0.04)), url('${auth_background}')`,
            minHeight: 560,
          }}
        >
        ...
        <div className="relative w-full md:w-1/2 bg-white dark:bg-[#0b0b0b] p-10 md:p-16 rounded-tr-2xl rounded-br-2xl">
          ...
          <div className="mt-6 text-center text-sm text-zinc-500">
            ...
            <div className="flex items-center justify-center gap-4">
              <button aria-label="facebook" className="p-2 rounded-full bg-blue-600 text-white"><FaFacebook /></button>
              <button aria-label="google" className="p-2 rounded-full bg-white border"><FcGoogle /></button>
              <button aria-label="apple" className="p-2 rounded-full bg-black text-white"><FaApple /></button>
            </div>
          </div>
