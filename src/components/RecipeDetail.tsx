  ...
  return (
    <article className="w-full max-w-6xl mx-auto p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div className="flex items-center gap-6 text-sm text-zinc-600 dark:text-zinc-400">
          ...
          <div className="hidden sm:flex items-center gap-4">
            <div className="flex items-center gap-2">
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M12 8a4 4 0 100 8 4 4 0 000-8z"/></svg>
            ...
            <div className="flex items-center gap-2">
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M3 6h18v12H3z"/></svg>
        ...
        <div className="flex items-center gap-3">
          <button className="rounded-full bg-zinc-100 dark:bg-zinc-800 p-3"><IoPrintOutline /></button>
          <button className="rounded-full bg-zinc-100 dark:bg-zinc-800 p-3"><IoShareOutline /></button>
        </div>
      </div>
        ...
        <div className="lg:col-span-2">
          ...
          <div className="relative rounded-2xl overflow-hidden mb-6 shadow-md">
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <button className="pointer-events-auto h-16 w-16 rounded-full bg-white/90 dark:bg-black/70 flex items-center justify-center text-2xl shadow-lg">▶</button>
            </div>
          </div>

          {/* description */}
          <p className="text-zinc-700 dark:text-zinc-300 mb-8 leading-relaxed">
            {r.content ?? "A quick and tasty fried rice recipe perfect for weeknights. Combine cooled rice with scrambled eggs and stir-fried vegetables, seasoning to taste."}
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            ...
            <div>
              ...
              <div className="space-y-3">
                {otherRecipes.map((o) => (
                  <Link key={o.id} href={`/recipes/${o.id}`} className="flex items-center gap-3 bg-white dark:bg-[#071018] p-3 rounded-lg shadow-sm hover:shadow-md">
              ...
              <div className="mt-6 hidden md:block">
                <div className="rounded-lg overflow-hidden">
                  <Image src="/images/recipes/8.jpg" alt="promo" width={300} height={180} className="object-cover w-full" />
                </div>
              </div>
            </div>
          </div>
                ...
                <li key={d.id} className="flex flex-col gap-4">
                  ...
                  {d.img && (
                    <div className="rounded overflow-hidden">
                      <Image src={d.img} alt={`step ${d.id}`} width={1200} height={600} className="object-cover w-full rounded-lg" />
                    </div>
                  )}
                </li>
              ))}
          ...
          <div className="sticky top-28 space-y-6">
            <div className="bg-[#eaf9fb] dark:bg-[#072026] rounded-2xl p-6 shadow-sm">
              ...
              <ul className="text-sm text-zinc-700 dark:text-zinc-300 space-y-2">
                <li className="flex justify-between"><span>Calories</span><span>{r.nutrition?.calories ?? "219.9 kcal"}</span></li>
                <li className="flex justify-between"><span>Total Fat</span><span>{r.nutrition?.fat ?? "10.7 g"}</span></li>
                <li className="flex justify-between"><span>Protein</span><span>{r.nutrition?.protein ?? "7.9 g"}</span></li>
                <li className="flex justify-between"><span>Carbohydrate</span><span>{r.nutrition?.carbs ?? "22.3 g"}</span></li>
                <li className="flex justify-between"><span>Cholesterol</span><span>{r.nutrition?.cholesterol ?? "37.4 mg"}</span></li>
              </ul>
